...
export function LocationPicker({ onLocationChange }: LocationPickerProps) {
  ...
  async function reverseGeocode(lat: number, lon: number) {
    try {
      ...
      if (!res.ok) throw new Error(`HTTP error! status: ${res.status}`)
      const data = await res.json()
      return data.results[0]?.formatted || `${lat.toFixed(4)}, ${lon.toFixed(4)}`
    } catch (e) {
  ...
  function handleLocationSelect(location: Location) {
    ...
    setSearchTerm(location.name || `${location.lat.toFixed(4)}, ${location.lon.toFixed(4)}`)
    setShowResults(false)
  ...
  return (
    <div ref={containerRef} className="space-y-4">
          ...
          {showResults && searchResults.length > 0 && (
            <div className="space-y-2 max-h-48 overflow-y-auto bg-white border border-gray-200 rounded-lg p-2">
              {searchResults.map((result, index) => (
                ...
                >
                  <div className="font-medium text-gray-900 text-sm">{result.name?.split(",")[0]}</div>
      ...
      {inputMode === "map" && (
        <div className="space-y-3">
          <div className="text-sm text-gray-400 mb-2">Click on the map to select a location</div>
          <MapComponent
